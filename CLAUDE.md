...
- **Sleep** is stored as durations — `time_in_bed_minutes`, `actual_sleep_minutes` — plus the optional clock times `bed_time` and `wake_time` as `TimeOnly`, which is what the regularity metric needs. Sending both times without a duration fills the duration in (wrapping over midnight); a duration that was sent explicitly always wins. Efficiency is derived, never stored. Quality (the percentage Sleep Cycle reports) and the sleep-debt metric were both removed, column and all: one is a score that stopped being tracked, the other measured a backlog nobody acts on. Clock arithmetic on the client runs on a noon-anchored axis (`clockToNightAxis` in `lib/stats.ts`) so 23:50 and 00:10 are twenty minutes apart, and regularity is the spread of the sleep midpoint.
