...
- **Energy needs** are answered twice, in `frontend/src/lib/energy.ts`: a Mifflin-St Jeor formula from the body data on `AppUser` (`birth_date`, `height_cm`, `sex`, `activity_level` — all optional, edited under Profile → Body data), and a measured estimate from mean intake against the weight trend, which only appears once there are 14 logged calorie days and 8 weigh-ins. The measured number wins wherever it exists; the formula stands in until then. Both feed the balance and cut-target tiles under Metrics → Body.
