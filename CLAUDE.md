...
- **Charts and metrics.** There is one chart component (`frontend/src/components/charts/Chart.tsx`) covering lines, bars, the goal line and the trailing-average overlay; `Sparkline`, `StatTile`, `BarRow` and `MacroSplit` sit next to it. All the arithmetic — moving averages, regression slope, adherence, weekly buckets — lives in `frontend/src/lib/stats.ts` and is unit tested; components do not calculate. Series colours are the module colours from `lib/modules.ts`, never the accent. `MetricsPage` is a tab per area (`pages/metrics/sections/*`). Two switches, on purpose: `RangeTabs` + `useRange` (7/30/90/365 days) sets how far a *series* reaches back, and `PeriodTabs` + `usePeriod` (`lib/period.ts`) sets which *period* the counted numbers on the overview are totalled over. Both are stored per key in localStorage.
