...
- **Ingest token**: `POST /api/ingest/activity` takes `{entries:[{date,steps}]}` with an `X-Ingest-Token` (or `Bearer`) header and upserts step counts only — it never touches the hand-entered cardio answer. The token is issued at `POST /api/auth/ingest-token`, stored as a SHA-256 hash (`IngestTokens`), shown once, and replaceable but not recoverable. It exists so an iOS Shortcut can post Apple Health steps nightly.
